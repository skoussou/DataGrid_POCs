...
//		cm = new DefaultCacheManager(global.build(), config.build());
		...
		.clusteredDefault().transport().transport(muxTransport).clusterName("RDS-DATAGRID-CLUSTER").nodeName(System.getProperty("jboss.node.name")).addProperty(JGroupsTransport.CHANNEL_LOOKUP, MuxChannelLookup.class.getName())
		.serialization().classResolver(HybridClassResolver.getInstance(HybridCluster.class.getClassLoader()))
		.globalJmxStatistics().cacheManagerName(System.getProperty("jboss.node.name")).allowDuplicateDomains(true).enable()
		.build());
