...
import com.askitis.consulting.jdg.loader.LoadDataRunable;
 ...
 */
@WebService(serviceName = "RdsJDGDataLoadService", portName = "RdsJDGDataLoad", name = "RdsJDGDataLoad", endpointInterface = "com.askitis.consulting.ws.RdsJDGDataLoadService", targetNamespace = "http://www.askitis.com/jdg/load")
public class RdsJDGDataLoadServiceImpl implements RdsJDGDataLoadService {
...
////		Query q = qf.from(DerivativeProduct.class).having("name").like("DerivProduct-1").toBuilder().build();
//		
...
//		org.apache.lucene.search.Query query = qb.keyword().onFields("name").matching("DerivProduct-2*").createQuery();
...
//		}
//
//		log.info("<---------------------------- END OF ENTRIES ---------------------------------->");
//				
//		log.info("Retrieved ["+list.size()+ "] entries from JDG in ["+elapsedQueryTime+"] millisecs on average to write to JDG ");
...
//			org.apache.lucene.search.Query query = qb.keyword().onFields("id", "type").matching("20001 Product-20001").createQuery();
			...
			list = q.list();
			log.info("<-----------["+list.size()+"]----------------- FOUND ENTRIES for search onFields(\"name\").matching(\"DerivProduct-2\")---------------------------------->");
