...
/**
 * RDS Embedded Manager (Capacity-FACTOR=1)
 * @author skoussou
 *
 */
@ApplicationScoped
...
//				        .addProperty("hibernate.search.default.exclusive_index_use", "false")     
	...
	private EmbeddedCacheManager createCacheManagerProgramaticallyMultipleCaches() {
		...
		DefaultCacheManager manager = new DefaultCacheManager(
				...
				.transport().addProperty("configurationFile", "jgroups-udp.xml").clusterName("RDS-DATAGRID-CLUSTER").nodeName("RDS-EAP-JDG-1")    // Transport defined by JGROUPS xml configuration over UDP, cluster name RDS-DATAGRID-CLUSTER
				.build());
...
//				"default").create();
		...
		//System.out.println("Node %s joined as master["+CacheManagerProvider.CACHE_NAMES.RDS_SYNC_WRITE_THROUGH.name()+"]. View is %s.%n", transport.getAddress(), transport.getMembers());
		return manager;
	}
...
//				.build());
		...
		DefaultCacheManager manager = new DefaultCacheManager(
		new GlobalConfigurationBuilder().clusteredDefault().transport().transport(muxTransport).clusterName("RDS-DATAGRID-CLUSTER").nodeName("Loader")
...
//						.addProperty("default.indexBase", "/tmp/JDG_INDEX")
		...
		//.hash().capacityFactor(1f)// TODO - NOTE THIS IS SET TO CAPACITY-FACTOR=1 it plays the role of the remote Data aware cache
		.sync()
